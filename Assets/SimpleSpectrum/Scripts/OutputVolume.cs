...
public class OutputVolume : MonoBehaviour
{
    ...
    public Vector3 valueMultiplier = new(0, 0, -90);
    ...
    private void Start()
    {
        if (outputType == OutputType.PrefabBar)
        {
            ...
            var rend = barT.GetChild(0).GetComponent<Renderer>();
            if (rend != null)
            {
                mat = rend.material;
            }
            ...
            {
                var img = barT.GetChild(0).GetComponent<Image>();
                if (img != null)
                {
                ...
                {
                    Debug.LogWarning(
                        "Warning from OutputVolume: The Bar Prefab you're using doesn't have a Renderer or Image component as its first child. Dynamic colouring will not work.");
    ...
    private void Update()
    {
        ...
        var newScale = newValue > oldScale
            ? Mathf.Lerp(oldScale, newValue, attackDamp)
            : Mathf.Lerp(oldScale, newValue, decayDamp);
    ...
    public static float GetRMS(AudioSource aSource, int sampleSize, int channelUsed = 0)
    {
#if UNITY_WEBGL
        Debug.LogError(
            "Error from SimpleSpectrum: You can't use OutputVolume.GetRMS against a single AudioSource in WebGL!");
        return 0;
#endif
        ...
        float rms = 0;
        foreach (var f in outputSamples) rms += f * f; //sum of squares
