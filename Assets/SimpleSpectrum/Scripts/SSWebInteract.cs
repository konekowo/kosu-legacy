...
public static class SSWebInteract
{
    ...
    private static byte[]
        freqBytesThisFrame; //since we can only use one Analyser, we might as well cache the result from the jslib.
    ...
    public static void GetSpectrumData(float[] samples)
    {
        ...
        for (var i = 0; i < len; i++) //populate the samples array
        {
    ...
    public static int
        SetFFTSize(int numSamples) //sets the FFT size, and afterwards returns the currently set FFT size (since we're only allowed one).
    {
    ...
    public static float GetLoudness()
    {
        if (loudnessTime != Time.realtimeSinceStartup) //if loudness hasn't yet been calculated for this frame,
        {
            ...
            var len = loudnessBytes.Length;
            var tot = 0;
            for (var i = 0; i < len; i++)
                tot += Mathf.Abs(loudnessBytes[i] - 128); //soo technically this isn't RMS... but we'll see how it goes
            loudnessThisFrame =
                tot / (float)(len * 128); //take mean, and divide by 128 cause the byte loudness is in range 0-128
