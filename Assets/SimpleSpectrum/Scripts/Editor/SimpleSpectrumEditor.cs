...
public class SimpleSpectrumEditor : Editor
{
    ...
    private SerializedProperty propertyNumSamples;
    private SerializedProperty propertySampleChannel;
    private SerializedProperty propertyUseLogFreq;
    private SerializedProperty propertyMultiplyByFreq;

    private SerializedProperty propertyFrequencyLimitLow;
    private SerializedProperty propertyFrequencyLimitHigh;

    private SerializedProperty propertyBarPrefab;
    private SerializedProperty propertyBarAmount;
    private SerializedProperty propertyBarYScale;
    private SerializedProperty propertyBarMinYScale;
    private SerializedProperty propertyBarXScale;
    private SerializedProperty propertyBarXSpacing;
    private SerializedProperty propertyBarCurveAngle;
    private SerializedProperty propertyBarXRotation;

    private SerializedProperty propertyUseColorGradient;
    private SerializedProperty propertyColorMin;
    private SerializedProperty propertyColorMax;
    ...
    private bool foldoutSpectrumOpen = true;
    private bool foldoutSamplingOpen = true;
    private bool foldoutBarsOpen = true;
    private bool foldoutColorsOpen = true;
    ...
    {
        if (((SimpleSpectrum)target).muteGroup == null) ((SimpleSpectrum)target).muteGroup = muteGroup;
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.LabelField(
            "NOTE: SimpleSpectrum works with WebGL, but only under certain conditions. Check the docs!",
        ...
        if (foldoutSpectrumOpen)
        {
            ...
            if (propertySourceType.enumValueIndex == 0) EditorGUILayout.PropertyField(propertyAudioSource);

            if (propertySourceType.enumValueIndex == 4)
                EditorGUILayout.LabelField(
                    "Use the spectrumInputData property to set your own data. It's probably worth disabling 'Use Logarithmic Frequency'.",
        ...
        {
#if UNITY_WEBGL
            EditorGUILayout.LabelField("Careful when using Color Gradients in WebGL, it can damage performance.",
                new GUIStyle { wordWrap = true });
#endif
            EditorGUILayout.PropertyField(propertyUseColorGradient);

            EditorGUILayout.PropertyField(propertyColorMin,
                new GUIContent(propertyUseColorGradient.boolValue ? "Minimum Color" : "Color"));
        ...
        if (GUILayout.Button("Rebuild Spectrum")) Rebuild();
    ...
    {
        if (Application.isPlaying) ((SimpleSpectrum)target).RebuildSpectrum();
    }
}