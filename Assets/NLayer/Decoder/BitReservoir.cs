...
namespace NLayer.Decoder
{
    internal class BitReservoir
    {
        ...
        public int Get1Bit()
        {
            ...
            if (_bitsLeft == 0 && (_start = (_start + 1) % _buf.Length) != _end + 1) _bitsLeft = 8;
        ...
        public int TryPeekBits(int count, out int readCount)
        {
            ...
            while (count > 0)
            {
                ...
                if ((resStart = (resStart + 1) % _buf.Length) == _end + 1) break;
        ...
        public int BitsAvailable
        {
            get
            {
                if (_bitsLeft > 0) return (_end + _buf.Length - _start) % _buf.Length * 8 + _bitsLeft;
                return 0;
            }
        }
        ...
        public void RewindBits(int count)
        {
            ...
            while (_start < 0) _start += _buf.Length;
        }

        public void FlushBits()
        {
            if (_bitsLeft < 8) SkipBits(_bitsLeft);
        }
