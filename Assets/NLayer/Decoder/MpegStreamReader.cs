...
namespace NLayer.Decoder
{
    internal class MpegStreamReader
    {
        ...
        internal MpegStreamReader(Stream source)
        {
            ...
            while (frame != null && !(frame is MpegFrame)) frame = FindNextFrame();
                        ...
                        do
                        {
                            ...
                            if (frame != null)
                                   ...
                                   )
                                {
                                    ...
                                    {
                                        if (frame.SampleCount != _first.SampleCount) _mixedFrameSize = true;
                                    ...
                                    if (frame.BitRateIndex == 0) _lastFree = frame;
                    ...
                    if (freeFrame != null)
                    {
                        ...
                        if (!_canSeek)
                            ...
                            throw new InvalidOperationException(
                                "Free frames cannot be read properly from forward-only streams!");
        ...
        private class ReadBuffer
        {
            ...
            public int ReadByte(MpegStreamReader reader, long offset)
            {
                lock (_localLock)
                {
                    var count = 1;
                    var startIdx = EnsureFilled(reader, offset, ref count);
                    if (count == 1) return Data[startIdx];
                }
            ...
            private int EnsureFilled(MpegStreamReader reader, long offset, ref int count)
            {
                ...
                if (startIdx < 0 || endIdx > End)
                {
                    ...
                    if (endIdx - moveCount > Data.Length || readStart + readCount - moveCount > Data.Length)
                    {
                        ...
                        while (newSize < endIdx - moveCount) newSize *= 2;
                    ...
                    else if (moveCount != 0)
                    {
                        ...
                        {
                            // backward move
                            for (int i = 0, srcIdx = Data.Length - 1, destIdx = Data.Length - 1 - moveCount;
                                 i < moveCount;
                                 i++, srcIdx--, destIdx--) Data[destIdx] = Data[srcIdx];
        ...
        private ReadBuffer _readBuf = new(2048);
        ...
        internal void ReadToEnd()
        {
            try
            {
                ...
                if (_id3Frame != null) maxAllocation += _id3Frame.Length;
