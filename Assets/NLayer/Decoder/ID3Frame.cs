...
namespace NLayer.Decoder
{
    internal class ID3Frame : FrameBase
    {
        ...
        {
            if ((syncMark & 0xFFFFFF00U) == 0x49443300) return new ID3Frame { _version = 2 };
            ...
            switch (_version)
            {
                case 2:
                    ...
                    if (Read(3, buf) == 7)
                    {
                        ...
                        if (!(((buf[2] & flagsMask) | (buf[3] & 0x80) | (buf[4] & 0x80) | (buf[5] & 0x80) |
                               (buf[6] & 0x80)) != 0 ||
                              buf[1] == 0xFF)) return size + 10; // don't forget the sync, flag & size bytes!
                    }
