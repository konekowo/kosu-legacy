...
namespace NLayer.Decoder
{
    internal class Huffman
    {
        ...
        static Huffman()
        {
            ...
            for (var i = 0; i < 8207; i++) _floatLookup[i] = (float)Math.Pow(i, 4.0 / 3);
        }
        ...
        private static byte DecodeSymbol(BitReservoir br, int table)
        {
            ...
            int readBits;
            var bits = br.TryPeekBits(maxBits, out readBits);
            if (readBits < maxBits) bits <<= maxBits - readBits;
        ...
        private static HuffmanListNode BuildLinkedList(List<byte> values, List<int> lengthList, List<int> codeList,
            out int maxBits)
        {
            ...
            for (var i = 1; i < list.Length && list[i].Length < 99999; i++) list[i - 1].Next = list[i];
