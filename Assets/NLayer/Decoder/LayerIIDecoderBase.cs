...
namespace NLayer.Decoder
{
    ...
    internal abstract class LayerIIDecoderBase : LayerDecoderBase
    {
        ...
        protected static bool GetCRC(MpegFrame frame, int[] rateTable, int[][] allocLookupTable, bool readScfsiBits,
            ref uint crc)
        {
            ...
            if (frame.ChannelMode == MpegChannelMode.JointStereo) jsbound = frame.ChannelModeExtension * 4 + 4;
        ...
        {
            0.00000000000f,
            0.00000000000f, 1.33333333333f, 1.14285714286f, 1.06666666666f, 1.03225806452f, 1.01587301587f,
            1.00787401575f, 1.00392156863f,
            1.00195694716f, 1.00097751711f, 1.00048851979f, 1.00024420024f, 1.00012208522f, 1.00006103888f,
        ...
        {
            0.00000000000f - 0f,
            0.00000000000f - 0f, 0.50000000000f - 1f, 0.25000000000f - 1f, 0.12500000000f - 1f, 0.062500000000f - 1f,
            0.03125000000f - 1f, 0.01562500000f - 1f, 0.00781250000f - 1f,
            0.00390625000f - 1f, 0.00195312500f - 1f, 0.00097656250f - 1f, 0.00048828125f - 1f, 0.000244140630f - 1f,
            0.00012207031f - 1f, 0.00006103516f - 1f, 0.00003051758f - 1f
        };
        ...
        private static readonly float[] _denormalMultiplier =
        {
            ...
            0.04960628287401f, 0.03937253280921f, 0.03125000000000f, 0.02480314143700f, 0.01968626640461f,
            0.01562500000000f, 0.01240157071850f, 0.00984313320230f,
            0.00781250000000f, 0.00620078535925f, 0.00492156660115f, 0.00390625000000f, 0.00310039267963f,
            0.00246078330058f, 0.00195312500000f, 0.00155019633981f,
            0.00123039165029f, 0.00097656250000f, 0.00077509816991f, 0.00061519582514f, 0.00048828125000f,
            ...
            0.00000480621738f, 0.00000381469727f, 0.00000302772723f, 0.00000240310869f, 0.00000190734863f,
            0.00000151386361f, 0.00000120155435f, 0.00000095367432f
        };
            ...
            : base()
        {
            ...
            _samples = new int[][]
                { new int[SBLIMIT * SSLIMIT * _granuleCount], new int[SBLIMIT * SSLIMIT * _granuleCount] };
        ...
        internal override int DecodeFrame(IMpegFrame frame, float[] ch0, float[] ch1)
        {
            ...
            for (var i = 0; i < _scfsi[0].Length; i++)
            {
        ...
        private void ReadAllocation(IMpegFrame frame, int[] rateTable)
        {
            ...
            for (; sb < _jsbound; sb++)
            {
                ...
                var bits = table[0];
                for (var ch = 0; ch < _channels; ch++) _allocation[ch][sb] = table[frame.ReadBits(bits) + 1];
            }
        ...
        private void ReadSamples(IMpegFrame frame)
        {
            ...
            for (var sb = 0; sb < SBLIMIT; sb++, idx++)
            for (var ch = 0; ch < _channels; ch++)
                if (ch == 0 || sb < _jsbound)
                {
                    ...
                    if (alloc != 0)
                    {
                        ...
                        {
                            // non-grouping
                            for (var gr = 0; gr < _granuleCount; gr++)
                    ...
                    else
                    {
                        ...
                        for (var gr = 0; gr < _granuleCount; gr++) _samples[ch][idx + SBLIMIT * gr] = 0;
                    }
                }
                else
                {
                ...
                for (var ss = 0; ss < SSLIMIT; ss++)
                {
                    for (var sb = 0; sb < SBLIMIT; sb++, idx++)
                    {
                        ...
                        if (alloc != 0)
                        {
                            ...
                            _polyPhaseBuf[sb] = c[alloc] * ((_samples[ch][idx] << (16 - alloc)) / 32768f + d[alloc]) *
