    ...
    internal class MpegFrame : FrameBase, IMpegFrame
    {
        ...
        internal static void UpdateCRC(int data, int length, ref uint crc)
        {
            ...
            while ((masking >>= 1) != 0)
            {
                ...
                crc <<= 1;
                if ((carry == 0) ^ ((data & masking) == 0)) crc ^= 0x8005;
            }
        ...
        internal VBRInfo ParseVBR()
        {
            ...
            if (buf[0] == 'V' && buf[1] == 'B' && buf[2] == 'R' && buf[3] == 'I') return ParseVBRI();
        ...
        public override string ToString()
        {
            ...
            if (flagList.Count > 0) sb.AppendFormat(" ({0})", string.Join(",", flagList.ToArray()));
