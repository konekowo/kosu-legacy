...
namespace NLayer
{
    public class MpegFrameDecoder
    {
        ...
        private int DecodeFrameImpl(IMpegFrame frame, Array dest, int destOffset)
        {
            ...
            switch (frame.Layer)
            {
                case MpegLayer.LayerI:
                    if (_layerIDecoder == null) _layerIDecoder = new Decoder.LayerIDecoder();
                    curDecoder = _layerIDecoder;
                    break;
                case MpegLayer.LayerII:
                    if (_layerIIDecoder == null) _layerIIDecoder = new Decoder.LayerIIDecoder();
                    curDecoder = _layerIIDecoder;
                    break;
                case MpegLayer.LayerIII:
                    if (_layerIIIDecoder == null) _layerIIIDecoder = new Decoder.LayerIIIDecoder();
        ...
        public void Reset()
        {
            ...
            if (_layerIDecoder != null) _layerIDecoder.ResetForSeek();
            if (_layerIIDecoder != null) _layerIIDecoder.ResetForSeek();
            if (_layerIIIDecoder != null) _layerIIIDecoder.ResetForSeek();
        }
    }
}