...
namespace NLayer
{
    public class MpegFile : IDisposable
    {
        ...
        {
            Init(
                System.IO.File.Open(fileName, System.IO.FileMode.Open, System.IO.FileAccess.Read,
        ...
        public long Position
        {
            ...
            set
            {
                ...
                lock (_seekLock)
                {
                    ...
                    if (sampleOffset != 0)
                    {
                        _decoder.DecodeFrame(_reader.NextFrame(), _readBuf,
                            0); // throw away a frame (but allow the decoder to resync)
                        newPos += sampleOffset;
                    }
            ...
            lock (_seekLock)
            {
                while (count > 0)
                {
                    if (_readBufLen > _readBufOfs)
                    {
                        ...
                        if (_readBufOfs == _readBufLen) _readBufLen = 0;
                    }
