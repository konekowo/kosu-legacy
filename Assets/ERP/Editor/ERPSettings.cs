...
namespace ERP
{
    ...
    public class ERPSettings
    {
        ...
        public static void GetSettings()
        {
            if (File.Exists(path))
            {
                var serializer = new XmlSerializer(typeof(ERPSettings));
                var stream = new FileStream(path, FileMode.Open);
                var settings = serializer.Deserialize(stream) as ERPSettings;
        ...
        {
            var settings = new ERPSettings(ERP.showSceneName, ERP.showProjectName, ERP.resetOnSceneChange,
